...
const ListUsers = () => {
  ...
  useEffect(() => {
    const fetchEmployees = async () => {
      ...
      try {
        const response = await axios.get('https://deployment-370a.onrender.com/api/user?role=employee', {
          headers: {
  ...
  const handleAddEmployee = async () => {
    const token = localStorage.getItem('authToken');
    try {
      const response = await axios.post('https://deployment-370a.onrender.com/api/auth/create-employee', newEmployee, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
